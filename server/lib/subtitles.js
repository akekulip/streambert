"use strict";
// ── lib: Subtitles (web port of src/ipc/subtitles.js) ─────────────────────────
// Subtitle search (SubDL + Wyzie), ZIP extraction, download-for-file, and
// subtitle-registry management. De-Electron-ified: no ipcMain / app.getPath /
// dialog. Uses Node built-ins only (global fetch, fs, path, zlib).
//
// The downloads registry (owned by the downloads module) is injected as a
// `store = { getDownloads, saveDownloads }` so this file never imports another
// agent's module. If no store is provided, registry merges are skipped but the
// on-disk subtitle files are still written/deleted and returned to the caller.
...
// ── Subtitle search (SubDL + Wyzie) ───────────────────────────────────────────
// args: { tmdbId, mediaType, season, episode, languages, subdlApiKey, wyzieApiKey }
...
// args: { fileId }; opts: { dataDir } (SubDL ZIPs are extracted into
// dataDir/subtitles and returned as a file:// URL for the server-side downloader).
async function getSubtitleUrl({ fileId } = {}, { dataDir } = {}) {
  try {
    if (String(fileId).startsWith("subdl_")) {
      ...
      const subDir = path.join(dataDir || process.cwd(), "subtitles");
...
async function downloadSubtitlesForFile(
  ...
  try {
    ...
    // Merge into download registry (if a store is wired).
    if (results.length > 0 && filePath && store) {
...
// ── Wyzie API-key validation (port of index.js "wyzie-validate-key") ──────────
async function wyzieValidateKey(key) {
  try {
    // Key goes as a query-param, not an Authorization header.
    const res = await fetchWithTimeout(
