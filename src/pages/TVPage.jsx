...
export default function TVPage({
  ...
  // Autoplay-next: refs let the progress tick reach the hook without re-subscribing.
    ...
    tmdbFetch(`/tv/${item.id}`, apiKey)
      .then((d) => {
        ...
        if (item.season == null) {
          ...
          // Open the lowest season that isn't fully watched
  ...
  // Reset auto-mark guard + autoplay countdown when episode changes
  useEffect(() => {
      ...
      interval = setInterval(async () => {
        try {
          ...
          if (result && result.duration > 0) {
            ...
            // Autoplay next episode when the current one is near its end.
            if (
              remaining <= watchedThreshold &&
  ...
  // Web fullscreen: fullscreen the player wrapper (embed iframe or <video>). On
  // the web build the Electron webview-fullscreen path above is a no-op, so this
  // is the only way to go fullscreen. (iOS Safari only fullscreens <video>.)
                ...
                {autoplayCountdown !== null && (
                  <div
                    style={{
                      ...
                      display: "flex",
                      justifyContent:
                        autoplayNextLayout === "left" ? "flex-start" : "flex-end",
                    ...
                    >
                      ...
                      >
                        {nextEp?.still_path ? (
                          ...
                          <span style={{ display: "inline-flex", width: 32, height: 32, color: "var(--text3)" }}>
                            <PlayIcon />
                          </span>
                        )}
                      </div>
